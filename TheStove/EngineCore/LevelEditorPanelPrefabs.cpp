...
void DrawPrefabsPanel(LevelEditor &editor, Scene &scene, int &selectedObjectId) {
    ...
    if (ImGui::Button("Instantiate from prefab")) {
        ...
        if (LoadPrefabFromFile(prefabPath, data)) {
            ...
            const std::string resolvedLayer = data.layer.empty() ? "1" : data.layer;
            ...
            if (g != nullptr) {
                ...
                if (data.hasCollider) {
                    ...
                    g->SetColliderSize({0.0f, 0.0f});
                    g->SetColliderOffset({0.0f, 0.0f});
                }

                g->EnableShadow(data.shadow);
                g->SetShadowSize(glm::vec2(data.w * 0.8f, data.h * 0.33f));
                g->SetShadowOffset(glm::vec2(0.0f, 55.0f));
                g->SetShadowOpacity(0.65f);

                // Store back into scene helpers for consistency
                scene.SetObjectTexturePath(id, data.texture);
                ...
                Scene::Defaults defs{};
                defs.pos = {data.x, data.y, data.z};
                defs.size = {data.w, data.h, 1.0f};
                defs.rot = data.rotation;
                defs.colSize = data.hasCollider ? Math::Vector2D{data.colWidth, data.colHeight} : Math::Vector2D{0.0f, 0.0f};
                defs.colOff = data.hasCollider ? Math::Vector2D{data.colOffsetX, data.colOffsetY} : Math::Vector2D{0.0f, 0.0f};
                defs.vel = {data.speedX, data.speedY};
                defs.texture = data.texture;
                defs.tag = data.tag;
                defs.layer = resolvedLayer;
                defs.approachOffset = {data.approachOffsetX, data.approachOffsetY};
                defs.hasApproachOffset2 = data.hasApproachOffset2;
                defs.approachOffset2 = {data.approachOffset2X, data.approachOffset2Y};
                defs.hasCustomerSeatOffset = data.hasCustomerSeatOffset;
                defs.customerSeatOffset = {data.customerSeatOffsetX, data.customerSeatOffsetY};
                defs.customerSeatCapacity = data.customerSeatCapacity;
                defs.hasCustomerSeatOffset2 = data.hasCustomerSeatOffset2;
                defs.customerSeatOffset2 = {data.customerSeatOffset2X, data.customerSeatOffset2Y};
                defs.audioOnSpawn = data.audioOnSpawn;
                defs.audioOnInteract = data.audioOnInteract;
                defs.audioOnDestroy = data.audioOnDestroy;
                defs.audioOnProcessing = data.audioOnProcessing;
                defs.audioLoop = data.audioLoop;
                defs.visible = data.visible;
                scene.SetDefaults(id, defs);

                scene.SetObjectTag(id, data.tag);
                scene.SetNPCVelocity(id, data.speedX, data.speedY);
                scene.SetObjectVisible(id, data.visible);

                // Re-assign special IDs via tag rules hook
                scene.ApplyTagRules(id, data.tag, data.speedX, data.speedY);
                scene.ApplyRuntimeObjectSetup(id, data.tag, data.texture, data.animated, data.animName, data.speedX, data.speedY);
                scene.AttachLogicForTag(id, data.tag);
                scene.MarkAnimated(id, data.animated);

                if (data.animated && !data.animName.empty() && scene.HasAnimations(id)) {
                    scene.SetAnimation(id, data.animName);
                } else if (!data.animated) {
                    g->SetUVRect({0.0f, 0.0f, 1.0f, 1.0f});
                }
                ...
                PrefabLinkByID[id] = NormalizePrefabPath(prefabPath);
