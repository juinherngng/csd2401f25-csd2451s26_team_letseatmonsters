...
/**
 * @brief Resolves a scene/runtime level path back to the repo-root source level JSON.
 * @param rawPath Scene-reported level path, which may be build-relative or already absolute.
 * @return Best-effort absolute source path under repo_root/levels, or an empty path on failure.
 */
static std::filesystem::path ResolveEditorSourceLevelPath(const std::string& rawPath) {
	namespace fs = std::filesystem;
	...
	if (input.is_absolute() && fs::exists(input, ec) && !ec) {
		return input.lexically_normal();
	}

	const fs::path repoLevelsDir = FindRepoRoot() / "levels";
	const fs::path fileName = input.filename();
	if (!fileName.empty()) {
		const fs::path repoCandidate = (repoLevelsDir / fileName).lexically_normal();
		if (fs::exists(repoCandidate, ec) && !ec) {
	...
	const std::vector<fs::path> candidates{
		(exeDir / input).lexically_normal(),
		(exeDir.parent_path() / input).lexically_normal(),
		(fs::current_path() / input).lexically_normal() };

	for (const fs::path& candidate : candidates) {
		if (fs::exists(candidate, ec) && !ec) {
			if (!candidate.filename().empty()) {
				const fs::path repoCandidate = (repoLevelsDir / candidate.filename()).lexically_normal();
				if (fs::exists(repoCandidate, ec) && !ec) {
					return repoCandidate;
				}
			}
...
namespace {
	...
	[[maybe_unused]] static bool InferEditorPlaySimulationActive(const std::string& levelPath) {
		...
		if (lower.find("main_menu") != std::string::npos ||
			lower.find("settings") != std::string::npos ||
...
namespace LEPANELLEVEL {
	...
	void DrawLevelPanel(LevelEditor& editor, Scene& scene,
		...
		static std::string sLastBoundSceneLevelPath;

		const std::filesystem::path resolvedSceneSourcePath = ResolveEditorSourceLevelPath(scene.GetCurrentLevelPath());
		const std::string resolvedSceneSourcePathString = resolvedSceneSourcePath.generic_string();
		if (!resolvedSceneSourcePathString.empty() && resolvedSceneSourcePathString != sLastBoundSceneLevelPath) {
			if (editor.levelPath.empty() || editor.levelPath == sLastBoundSceneLevelPath) {
				editor.levelPath = resolvedSceneSourcePathString;
				const std::string stem = resolvedSceneSourcePath.stem().string();
				std::snprintf(levelPathBuf, sizeof(levelPathBuf), "%s", stem.c_str());
			}
			sLastBoundSceneLevelPath = resolvedSceneSourcePathString;
		}
...
LevelData& prePlaySnapshot = editor.MutablePlaySnapshot();
CaptureEditorState(scene, prePlaySnapshot);
...
// Keep Stop restore behavior tied to the pre-play snapshot,
// but load Play from disk for runtime parity with release.
LevelData playData = prePlaySnapshot;
if (!playLevelPath.empty()) {
	LevelData diskLevel{};
	if (LevelSerializer::LoadExact(playLevelPath, diskLevel)) {
		playData = diskLevel;
	}
	else {
		TS_LOG_WARN("[LevelEditor] Play fallback to in-memory snapshot; exact load failed for '" << playLevelPath << "'");
	}
}
...
ApplyLevelToEditorScene(scene, playLevelPath, playData, playSimulationActive); }, [&]() {
editor.SetPlaying(false);
StopEditorAudio(scene);
scene.SetSimulationActive(false);
...
LEHIERARCHY::InvalidateCache(); } });
		...
		if (ImGui::Button("Add##ObjectAdd")) {
			...
			if (GameObject* obj = scene.SpawnStaticSprite(proto.texture, { proto.x, proto.y, proto.z }, { proto.w, proto.h }, proto.layer)) {
				...
				obj->EnableShadow(proto.shadow);
				obj->SetShadowSize(glm::vec2(proto.w * 0.8f, proto.h * 0.33f));
				obj->SetShadowOffset(glm::vec2(0.0f, 55.0f));
				obj->SetShadowOpacity(0.65f);

				scene.SetObjectTexturePath(id, proto.texture);
				scene.AssignObjectToLayer(id, proto.layer);
				scene.SetTransformFromLevel(id, { proto.x, proto.y, proto.z }, { proto.w, proto.h, 1.0f }, proto.rotation);

				Scene::Defaults defs{};
				defs.pos = { proto.x, proto.y, proto.z };
				defs.size = { proto.w, proto.h, 1.0f };
				defs.rot = proto.rotation;
				defs.colSize = proto.hasCollider ? Math::Vector2D{ proto.colWidth, proto.colHeight } : Math::Vector2D{ 0.0f, 0.0f };
				defs.colOff = proto.hasCollider ? Math::Vector2D{ proto.colOffsetX, proto.colOffsetY } : Math::Vector2D{ 0.0f, 0.0f };
				defs.vel = { proto.speedX, proto.speedY };
				defs.approachOffset = { proto.approachOffsetX, proto.approachOffsetY };
				defs.hasApproachOffset2 = proto.hasApproachOffset2;
				defs.approachOffset2 = { proto.approachOffset2X, proto.approachOffset2Y };
				defs.hasCustomerSeatOffset = proto.hasCustomerSeatOffset;
				defs.customerSeatOffset = { proto.customerSeatOffsetX, proto.customerSeatOffsetY };
				defs.customerSeatCapacity = proto.customerSeatCapacity;
				defs.hasCustomerSeatOffset2 = proto.hasCustomerSeatOffset2;
				defs.customerSeatOffset2 = { proto.customerSeatOffset2X, proto.customerSeatOffset2Y };
				defs.texture = proto.texture;
				defs.tag = proto.tag;
				defs.layer = proto.layer;
				defs.audioOnSpawn = proto.audioOnSpawn;
				defs.audioOnInteract = proto.audioOnInteract;
				defs.audioOnDestroy = proto.audioOnDestroy;
				defs.audioOnProcessing = proto.audioOnProcessing;
				defs.audioLoop = proto.audioLoop;
				defs.visible = proto.visible;
				scene.SetDefaults(id, defs);

				scene.SetObjectTag(id, proto.tag);
				scene.SetNPCVelocity(id, proto.speedX, proto.speedY);
				scene.SetObjectVisible(id, proto.visible);
				scene.ApplyTagRules(id, proto.tag, proto.speedX, proto.speedY);
				scene.ApplyRuntimeObjectSetup(id, proto.tag, proto.texture, proto.animated, proto.animName, proto.speedX, proto.speedY);
				scene.AttachLogicForTag(id, proto.tag);
				scene.MarkAnimated(id, proto.animated);

				if (proto.animated && !proto.animName.empty() && scene.HasAnimations(id)) {
					scene.SetAnimation(id, proto.animName);
				}
				else {
		...
		if (selectedIndex >= 0 && selectedIndex < static_cast<int>(objectList.size()) && objectList[selectedIndex]) {
			...
			// Tag preset (acts like a quick logic/component assignment helper)
			ImGui::Text("Tag Preset");
			ImGui::NextColumn();
			FullWidthNext();
			{
				static const char* kTagPresets[] = {
					"",
					"player",
					"table",
					"work_table",
				...
				const char* currentTagPreview = (tagBuf[0] == '\0') ? "<none>" : tagBuf;
				if (ImGui::BeginCombo("##TagPreset", currentTagPreview)) {
					for (const char* preset : kTagPresets) {
						const char* display = (preset[0] == '\0') ? "<none>" : preset;
						const bool isSelected = (std::strcmp(tagBuf, preset) == 0);
						if (ImGui::Selectable(display, isSelected)) {
							PushUndoSnapshot(editor, scene);
							std::snprintf(tagBuf, IM_ARRAYSIZE(tagBuf), "%s", preset);
						}
